...
public class CamController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            xmove += Input.GetAxis("Mouse X") * speed; // 마우스의 좌우 이동량을 xmove 에 누적합니다.
            ymove -= Input.GetAxis("Mouse Y") * speed; // 마우스의 상하 이동량을 ymove 에 누적합니다.
        }
        transform.rotation = Quaternion.Euler(ymove, xmove, 0); // 이동량에 따라 카메라의 바라보는 방향을 조정합니다.
        player.transform.rotation = Quaternion.Euler(ymove, xmove, 0);
